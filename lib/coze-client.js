...
async function batchProcessQuestions(questions, progressCallback = null) {
  ...
  // 从环境变量获取并发数，默认5
  const maxConcurrency = parseInt(process.env.DATA_PROCESSOR_THREADS) || 5;
  console.log(`🔄 使用并发数: ${maxConcurrency}`);
  ...
  // 处理单个任务的包装函数
  const processTask = async (question, index) => {
    try {
  ...
  // 启动初始并发任务
  while (currentIndex < questions.length && processingQueue.length < maxConcurrency) {
    const question = questions[currentIndex];
    const index = currentIndex;
    const promise = processTask(question, index);
  ...
  while (processingQueue.length > 0) {
    ...
    for (let i = processingQueue.length - 1; i >= 0; i--) {
      const promise = processingQueue[i];
