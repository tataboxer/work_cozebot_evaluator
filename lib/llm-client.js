...
1. 准确率：回复内容是否准确回答了用户问题，是否解决了用户的查询需求，是否与科技馆业务目标高度贴合。${expectedAnswer ? "如有参考答案，请以参考答案为准评估准确性。" : ""}${contextText ? "考虑上下文连贯性，但不需要评判对话历史用assistant的回复" : ""} 评分1-100分。

2. 专业度：用词是否精准、术语是否正确、业务上下文是否符合科技馆场景的专业水准。${expectedAnswer ? "参考标准答案的专业表达方式。" : ""} 评分1-100分。

3. 语气合理：语气是否礼貌友好、风格是否匹配科技馆数字助手场景（亲切、引导性、专业但不生硬）。${expectedAnswer ? "参考标准答案的语气风格。" : ""} 评分1-100分。
